...
const App = ({ type, list, onPageRequest, onImgDeleted, isDataLoading, totalCount, style }: Props) => {
    ...
    return <>
        ...
        {/* 未登录，我的页面显示登录按钮 */}
        {!isDataLoading && <> {(type === MusicPageType.MY || type === MusicPageType.MY_THUMB_UP_LIST) && (!user || !user.secret) && <div className={css['no-more-tips']}>您还未登录，请先<a href={`/${process.env.NODE_ENV === 'development' ? 'login' : 'login/'}?redirect=/art`}> 登录 </a>  </div>}
            {type === MusicPageType.MY && (user.secret) && list.length === 0 && <div className={css['no-more-tips']}>暂无数据，<Link href='/'> 开始绘画！ </Link>  </div>}
            {type === MusicPageType.PUBLIC && list.length === 0 && <div className={css['no-more-tips']}>暂无数据，<Link href='/'> 开始绘画！ </Link>  </div>}
            {type === MusicPageType.MY_THUMB_UP_LIST && list.length === 0 && <div className={css['no-more-tips']}>暂无数据，<Link href='/'> 快去点赞一个作品吧！ </Link>  </div>}
        </>}
            ...
            <><div className="music-list-container">
                {list.map((musicInfo: ImgCardModel) => {
                    ...
                    return musics.map((music: MusicModel, index: number) => {
                        return <div key={music.id} className="my-music-item">
                            <MusicCard {...music} />
                        </div>
                    })
                    // <div key={musicInfo.id} className="masonry-item">
                    //     <MusicCard {...musics[0]} />
                    //     {musics[1] && <MusicCard {...musics[1]} />
                    //     }
                    // </div>
                })}
            </div>
