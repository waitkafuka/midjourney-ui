...
const QrCode: React.FC = () => {
    ...
    const currentTemplate = useMemo(() => {
        if (params.template_id) {
            const template = qrTemplates77.find((t) => t.id === params.template_id);
            if (template) {
                return template;
            }
        } else {
            ...
            <div className="code-options-box">
                <div className="art-form-item">
                    <div className="form-item-label">
                        ...
                        <Tooltip title="如果要生成文字或链接，直接在这里输入；如果要生成微信名片，请点击右侧“识别”按钮上传">
                ...
                <div className="advance-options-box" style={{ display: showOptions ? 'block' : 'none' }}>
                    ...
                    <div className="art-form-item horizontal">
                        ...
                        <Select
                            ...
                            style={{ width: 190, marginLeft: "10px" }}
                            onChange={v => {
                                setParams({ ...params, template_id: v })
                            }}
                            options={qrTemplates77.map(item => ({ value: item.id, label: item.name }))}
                        />
                    </div>
                    {/* 版本选择 */}
