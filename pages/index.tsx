...
const Index: React.FC = () => {
  const [inputValue, setInputValue] = useState("an asian woman poses for a portrait, in the style of luminous shadows, dark white and beige, onii kei , fluid photography, realistic yet romantic, smooth lines, angura kei");
  ...
  const checkUserAuth = () => {
    ...
    if (!(user.token_type === 0 || user.token_type === 3)) {
      messageApi.error('目前 midjourney 绘画功能仅对网站包月用户开放试用权限。', 10);
      return false;
    }
    return true;
  }
    ...
    if (newMessage.text) {
      ...
      await Imagine(
        ...
        (data: any) => {
          ...
          // 判断用户信息
          if (!(user.token_type === 0 && user.token === 3)) {
  ...
  const renderMessage = ({
    ...
    return (
      ...
      >
        ...
        {img && <a href={img} style={{ textDecoration: "underline" }} target="_blank">下载（服务器未保存您的记录，请注意及时下载图片！）</a>}
        {hasTag && (
          <Tag
            Data={["V1", "V2", "V3", "V4"]}
            onClick={(tag) => {
